...
node_labels_with_count = [
    f"{label} ({int(node_flow[i])})" for i, label in enumerate(all_labels)
]
node_hover = [
    f"<b>{label}</b><br>Flow count: {int(node_flow[i])}<extra></extra>"
    for i, label in enumerate(all_labels)
]
link_hover = [
    f"{all_labels[s]} → {all_labels[t]}<br>Count: {int(v)}<extra></extra>"
    for s, t, v in zip(link_src, link_tgt, link_val)
]
